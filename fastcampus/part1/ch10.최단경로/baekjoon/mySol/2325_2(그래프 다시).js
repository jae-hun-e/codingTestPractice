...
for (let i = 0; i < path.length - 1; i++) {
    const delGraph = [...graph.map((v) => [...v])];

    delGraph[path[i]] = delGraph[path[i]].filter((v) => v[0] !== path[i + 1]);
    delGraph[path[i + 1]] = delGraph[path[i + 1]].filter((v) => v[0] !== path[i]);

    const cost = dijkstra(1, delGraph).at(-1)[0];
