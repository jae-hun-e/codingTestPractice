...
for (let r = 0; r < row; r++) {
    for (let c = 0; c < col; c++) {
        if (map[r][c] === 1) dfs(r, c, ++island_cnt);
    }
}

// 경계선 확인 및 거리 계산
island_cnt--;
const distance = Array.from(new Array(island_cnt), () =>
    new Array(island_cnt).fill(Infinity)
);
    ...
    for (let c = 0; c < col; c++) {
        if (map[r][c] > 0) {
            for (let i = 0; i < 4; i++) {
                ...
                // 섬과 섬 사이 거리 저장
                distance[map[r][c] - 2][map[r2][c2] - 2] = Math.min(
                    d,
                    distance[map[r][c] - 2][map[r2][c2] - 2]
                );
            }
        }
    }
...
for (let i = 0; i < island_cnt; i++) {
    for (let j = 0; j < island_cnt; j++) {
        if (distance[i][j] != Infinity) bridge_candidate.push([distance[i][j], i, j]);
    }
}
...
for (let [d, a, b] of bridge_candidate) {
    if (bridge_cnt == island_cnt - 1) break;
